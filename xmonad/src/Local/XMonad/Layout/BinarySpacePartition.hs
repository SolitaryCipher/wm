...
module Local.XMonad.Layout.BinarySpacePartition (
  -- * Usage
  -- $usage
    emptyBSP
  , configurableBSP
...
splitFromRect :: Rational -> Rectangle -> Split
splitFromRect r (Rectangle _ _ w h)
  | w >= h = Split Vertical r
  | otherwise = Split Horizontal r

splitCurrent :: Rational -> Rectangle -> Zipper Split -> Maybe (Zipper Split)
splitCurrent rat r (Leaf _, []) = Just (Node (splitFromRect rat r) (Leaf 0) (Leaf 0), [])
splitCurrent rat r (Leaf _, crumb:cs) = Just (Node (splitFromRect rat r) (Leaf 0) (Leaf 0), crumb:cs)
splitCurrent rat r (n, []) = Just (Node (splitFromRect rat r) (Leaf 0) (toggleSplits n), [])
splitCurrent rat r (n, crumb:cs) = Just (Node (splitFromRect rat r) (Leaf 0) (toggleSplits n), crumb:cs)
...
data BSPConfig a = BSPConfig { selectedColor :: String
                             , splitRatio :: Rational
...
configurableBSP :: String -> Rational -> BSPConfig a
configurableBSP selectedColor' rat = BSPConfig selectedColor' rat emptyBSP
...
splitNth :: Rational -> Rectangle -> BinarySpacePartition a -> BinarySpacePartition a
splitNth _ _ (BinarySpacePartition _ _ _ Nothing) = makeBSP (Leaf 0)
splitNth rat r b =
  ...
  in doToNth (splitCurrent rat focusedRect) b
...
instance LayoutClass BSPConfig Window where
  doLayout bl r s = do
    let b  = bsp bl
    let b' = layout b
    b'' <- updateNodeRefC color b' (size b/=size b') r -- this is where coloring happens
    let rs = rectangles b'' r
        wrs = zip ws rs
    return (wrs, Just bl{bsp=b''{getOldRects=wrs}})
    where
      ws = W.integrate s
      l = length ws
      color = selectedColor bl
      layout bsp'
        | l == sz = bsp'
        | l > sz = layout $ splitNth ratio' r bsp'
        | otherwise = layout $ removeNth bsp'
        where sz = size bsp'
              ratio' = splitRatio bl

  --handleMessage b_orig m = fmap (makeLayout b_orig) $ handleMessage (bsp b_orig) m
  handleMessage b_orig m = makeLayout b_orig <$> handleMessage (bsp b_orig) m
    where 
        makeLayout _ Nothing = Nothing
        makeLayout bl (Just b) = Just bl{bsp=b}
...
instance LayoutClass BinarySpacePartition Window where
  doLayout b r s = do
    ...
    where
      ...
      layout bsp'
        | l == sz = bsp'
        | l > sz = layout $ splitNth 0.5 r bsp'
        | otherwise = layout $ removeNth bsp'
        where sz = size bsp'
