...
splitFromRect ratio (Rectangle _ _ w h)
  | w >= h = (Split Vertical ratio)
  | otherwise = (Split Horizontal ratio)

splitCurrent :: Rational -> Rectangle -> Zipper Split -> Maybe (Zipper Split)
splitCurrent ratio r (Leaf _, []) = Just (Node (splitFromRect ratio r) (Leaf 0) (Leaf 0), [])
splitCurrent ratio r (Leaf _, crumb:cs) = Just (Node (splitFromRect ratio r) (Leaf 0) (Leaf 0), crumb:cs)
splitCurrent ratio r (n, []) = Just (Node (splitFromRect ratio r) (Leaf 0) (toggleSplits n), [])
splitCurrent ratio r (n, crumb:cs) = Just (Node (splitFromRect ratio r) (Leaf 0) (toggleSplits n), crumb:cs)
...
configurableBSP selectedColor ratio = BSPConfig selectedColor ratio emptyBSP
...
splitNth ratio r b =
  ...
  in doToNth (splitCurrent ratio focusedRect) b
...
updateNodeRef b force r = updateNodeRefC "#00ff00" b force r
